    ...
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-white">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
        ...
        <div className="relative">
          ...
          <div className="absolute hidden w-48 bg-white divide-y divide-gray-100 rounded-lg shadow-md">
            <ul className="p-3 space-y-1 text-sm text-gray-700">
              {dateFilters.map((filter) => (
                <li key={filter} onClick={() => setDateFilter(filter)} className="cursor-pointer p-2 hover:bg-gray-100">
                  {filter}
                </li>
              ))}
            </ul>
          ...
          {filteredTickets.length > 0 ? (
            filteredTickets.map((ticket) => (
              <tr key={ticket.id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className={`px-6 py-4 ${ticket.status === "Open" ? "text-red-500" : ticket.status === "Pending" ? "text-yellow-500" : "text-green-500"}`}>
                  {ticket.status}
                </td>
                <td className={`px-6 py-4 ${ticket.priority === "High" ? "text-red-500" : ticket.priority === "Medium" ? "text-yellow-500" : "text-green-500"}`}>
                  {ticket.priority}
                </td>
